...
export const entityFacts = {
  ...
  product: {
    ...
    rag: 'Retrieval-augmented generation (RAG) uses sources the user attaches from the Library when Ask Prof Z writes, instead of writing from a generic chat window alone.',
  },
  founder: {
  ...
  howItWorks:
    'RflowZ works as a guided proposal workspace: start with Ask Prof Z onboarding, search and attach Library sources (including OpenAlex), synthesize literature with RAG, run citation integrity checks, then export DOCX or PDF (PPTX on higher plans when those plans launch).',
  differentiator:
    'RflowZ is a structured research-proposal workspace with Ask Prof Z, OpenAlex Library search, RAG, citation integrity checks, and document export — not a general-purpose chat window.',
  limitations:
